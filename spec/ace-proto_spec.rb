...
EOT
    ...
    it 'should be number string when it not match IOS acl literal' do
      aups = AceTcpProtoSpec.new( :number => 6633 )
      aups.to_s.should be_aclstr( '6633' )
    end

    it 'raise error when out of range port number' do
      lambda {
        aups = AceTcpProtoSpec.new( :number => 65536 )
      }.should raise_error( AclArgumentError )

      lambda {
        aups = AceTcpProtoSpec.new( :number => -1 )
      }.should raise_error( AclArgumentError )
    end
...
EOT
    ...
    it 'raise error when out of range port number' do
      lambda {
        aups = AceIpProtoSpec.new( :number => 256 )
      }.should raise_error( AclArgumentError )

      lambda {
        aups = AceIpProtoSpec.new( :number => -1 )
      }.should raise_error( AclArgumentError )
    end
