...
describe StandardAce do
  ...
  describe '#matches' do
    ...
    it 'shoud be true with match ip addr' do
      @sa.contains?(src_ip: @ip_match).should be_true
    end
    ...
    context 'tcp src/dst ip/port full spec test' do
      ...
      (0..(2**bit - 1)).each do |num|
        ...
        (0...bit).each do |b|
          ...
          opts[:src_operator] = :eq
          opts[:dst_operator] = :eq
          flag = flag << 1
        end
        tests.push(
          opts: opts,
...
#{each[:opts][:dst_ip]}:#{each[:opts][:dst_port]}" do
          if each[:res]
            @ea.contains?(each[:opts]).should be_true
          else
            @ea.contains?(each[:opts]).should be_false
          end
        end # it
      end # tests.each
