  ...
  describe "#add_entry_by_params" do
    before do
      ...
      @acl.add_entry_by_params(
        :action => 'deny',
        :protocol => 'tcp',
        :src => { :ipaddr => '192.168.3.3', :wildcard => '0.0.0.0' },
        :dst => {
          :ipaddr => '192.168.4.4', :wildcard => '0.0.0.255',
          :operator => 'gt',
          :port1 => AceUdpProtoSpec.new( :number => 32768 )
        }
      )
    end
...
EOS
  ...
  describe "#search_ace" do
    ...
    it 'should be match 2nd entry' do
      ace = @acl.search_ace( :protocol => 'tcp',
        :src_ip => '192.168.10.3', :src_port => 64332,
        :dst_ip => '192.168.4.5', :dst_port => 32889
      )
      ace.to_s.should be_aclstr("deny tcp host 192.168.10.3 192.168.4.0 0.0.0.255 gt 32768")
    end

    it 'should be last entry' do
      ace = @acl.search_ace( :protocol => 'udp',
        :src_ip => '192.168.10.3', :src_port => 64332,
        :dst_ip => '10.0.0.3', :dst_port => 33890
      )
      ace.to_s.should be_aclstr("deny ip any 10.0.0.0 0.0.0.255")
    end

    it 'should be nil if not found match entry' do
      @acl.search_ace( :protocol => 'udp',
        :src_ip => '192.168.10.3', :src_port => 62223,
        :dst_ip => '11.0.0.3', :dst_port => 33333
      ).should be_nil
    end
  end
end
...
EOS
  ...
  describe "#add_entry_by_params" do
    before do
      ...
      @acl.add_entry_by_params(
        :action => 'deny',
        :src => { :ipaddr => '192.168.4.4', :wildcard => '0.0.0.255' }
      )
    end
...
EOS
  ...
  describe "#search_ace" do
    ...
    it 'should be last entry' do
      ace = @acl.search_ace(
        :src_ip => '10.0.0.3', :src_port => 33890
      )
    ...
    it 'should be nil if not found match entry' do
      @acl.search_ace( :protocol => 'udp',
        :src_ip => '11.0.0.3', :src_port => 33333
      ).should be_nil
    end
  end
